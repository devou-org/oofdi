...
export default function AdminMedia() {
  ...
  const handleUpload = async (file, type) => {
    ...
    if (res.ok) {
      ...
      await saveMediaUrls({ imageUrl: type === "image" ? url : imageUrl, videoUrl: type === "video" ? url : videoUrl });
  ...
  const handleReset = async (type) => {
    ...
    if (res.ok) {
      if (type === "image") {
        ...
        setVideo(null); setVideoUrl("");
      }
      await saveMediaUrls({ imageUrl: type === "image" ? "" : imageUrl, videoUrl: type === "video" ? "" : videoUrl });
        ...
        <div className="mb-10">
          ...
          {!imageUrl ? (
            <>
              ...
              <button onClick={() => handleUpload(image, "image")}
            ...
            <div className="mt-4 space-y-2">
              <a href={imageUrl} className="text-blue-600 underline">{imageUrl}</a>
              <img src={imageUrl} alt="Uploaded" className="max-w-full rounded shadow" />
              <button onClick={() => handleReset("image")}
          ...
          {!videoUrl ? (
            <>
              <input type="file" accept="video/*" onChange={(e) => setVideo(e.target.files[0])}
                className="mb-3 border-2 border-[#FF1F52] p-2 rounded" />
              <button onClick={() => handleUpload(video, "video")}
                className="bg-[#FF1F52] text-white px-5 py-2 rounded hover:bg-[#e11e4a]">
                Upload Video
              </button>
            </>
            ...
            <div className="mt-4 space-y-2">
              <a href={videoUrl} className="text-blue-600 underline">{videoUrl}</a>
              <video controls src={videoUrl} className="max-w-full rounded shadow" />
              <button onClick={() => handleReset("video")}
                className="text-sm text-red-500 underline hover:text-red-700">
